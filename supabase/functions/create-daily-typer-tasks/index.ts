...
serve(async (_req) => {
  ...
  try {
    ...
    // 1. Find the most recent active typer set
    ...
    for (const user of users) {
      // 3. Check if user already has 5 tasks for today
      const { count: existingTaskCount } = await supabaseAdmin
        .from('tasks')
      ...
      if (existingTaskCount !== null && existingTaskCount >= 5) continue;
      ...
      const assignedTextIds = new Set(assignedTasks?.map(t => t.related_typing_text_id).filter(Boolean));
      const availableTextIds = textIdsInSet.filter(id => !assignedTextIds.has(id));

      // 5. Select 5 random texts to assign
      const textsToAssign = availableTextIds.sort(() => 0.5 - Math.random()).slice(0, 5);

      if (textsToAssign.length > 0) {
        const dueDate = new Date();
        dueDate.setHours(23, 59, 59, 999);

        for (const textId of textsToAssign) {
            tasksToInsert.push({
              title: `Daily Typing Challenge: ${currentSet.title}`,
              description: "Complete 5 daily typing tests to earn a bonus!",
              assigned_to: user.id,
              assigned_by: user.id, // System assigned
              status: 'pending',
              due_date: dueDate.toISOString(),
              task_type: 'typer',
              related_typing_text_id: textId,
            });
        }
      }
    }
