...
serve(async (req) => {
  ...
  try {
    ...
    const { error } = await supabaseAdmin
      .from('app_settings')
      .upsert({ key: 'global_offer', value: offerValue }, { onConflict: 'key' });
    
    if (error) throw error;

    return new Response(JSON.stringify({ message: "Global offer set successfully." }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
  } catch (error) {
